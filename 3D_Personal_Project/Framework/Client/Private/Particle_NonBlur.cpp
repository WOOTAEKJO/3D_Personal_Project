#include "..\Public\Particle_NonBlur.h"

#include "GameInstance.h"
#include "Bone.h"
#include "Player.h"

CParticle_NonBlur::CParticle_NonBlur(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CParticle(pDevice, pContext)
{
}

CParticle_NonBlur::CParticle_NonBlur(const CParticle_NonBlur& rhs)
	: CParticle(rhs)
{
}

HRESULT CParticle_NonBlur::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CParticle_NonBlur::Initialize(void* pArg)
{
	if (pArg == nullptr)
		return E_FAIL;
...
void CParticle_NonBlur::Priority_Tick(_float fTimeDelta)
{
	/*if (m_bKeep)
	{
		m_pTransformCom->Set_State(CTransform::STATE::STATE_POS, m_vPos);
...
void CParticle_NonBlur::Tick(_float fTimeDelta)
{
	if (m_bSprite)
	{
...
void CParticle_NonBlur::Late_Tick(_float fTimeDelta)
{
...
HRESULT CParticle_NonBlur::Render()
{
...
void CParticle_NonBlur::Load_FromJson(const json& In_Json)
{
	
	__super::Load_FromJson(In_Json);
}

HRESULT CParticle_NonBlur::Bind_ShaderResources()
{
...
HRESULT CParticle_NonBlur::Ready_Component()
{
...
CParticle_NonBlur* CParticle_NonBlur::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CParticle_NonBlur* pInstance = new CParticle_NonBlur(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CParticle_NonBlur");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CParticle_NonBlur::Clone(void* pArg)
{
	CParticle_NonBlur* pInstance = new CParticle_NonBlur(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CParticle_NonBlur");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CParticle_NonBlur::Free()
{
	__super::Free();
}
