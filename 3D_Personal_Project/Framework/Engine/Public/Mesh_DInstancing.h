...
class CMesh_DInstancing final : public CVIBuffer_Instancing
{
private:
	CMesh_DInstancing(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CMesh_DInstancing(const CMesh_DInstancing& rhs);
	virtual	~CMesh_DInstancing() = default;
...
public:
	static	CMesh_DInstancing* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext,
		CModel::TYPE eType, MESH MeshData, _fmatrix matPivot, CModel::BONES& pBones);
