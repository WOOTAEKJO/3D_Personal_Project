...
CJson_Utility::CJson_Utility()
{
}

HRESULT CJson_Utility::Initialize()
{
	return S_OK;
}
...
void CJson_Utility::Write_Float2(Value& pValue, const  _char* strName, _float2 vVec, Document& doc)
{
	pValue.AddMember(GenericStringRef < char>(strName), vVec.x, doc.GetAllocator());
	pValue.AddMember(GenericStringRef < char>(strName), vVec.y, doc.GetAllocator());
}
void CJson_Utility::Write_Float3(Value& pValue, const  _char* strName, _float3 vVec, Document& doc)
{
	pValue.AddMember(GenericStringRef < char>(strName), vVec.x, doc.GetAllocator());
	pValue.AddMember(GenericStringRef < char>(strName), vVec.y, doc.GetAllocator());
	pValue.AddMember(GenericStringRef < char>(strName), vVec.z, doc.GetAllocator());
	
}
void CJson_Utility::Write_Float4(Value& pValue, const  _char* strName, _float4 vVec, Document& doc)
{
	pValue.AddMember(GenericStringRef < char>(strName), vVec.x, doc.GetAllocator());
	pValue.AddMember(GenericStringRef < char>(strName), vVec.y, doc.GetAllocator());
	pValue.AddMember(GenericStringRef < char>(strName), vVec.z, doc.GetAllocator());
	pValue.AddMember(GenericStringRef < char>(strName), vVec.w, doc.GetAllocator());
}
void CJson_Utility::Write_uint3(Value& pValue, const  _char* strName, _uint3 iValue, Document& doc)
{
	pValue.AddMember(GenericStringRef < char>(strName), iValue.iX, doc.GetAllocator());
