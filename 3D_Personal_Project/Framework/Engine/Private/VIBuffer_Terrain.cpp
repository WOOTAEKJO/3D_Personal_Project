...
HRESULT CVIBuffer_Terrain::Initialize_ProtoType(LOADTYPE eType, const wstring& strFilePath)
{
	if (eType == LOADTYPE::TYPE_HEIGHT)
	{
		Init_HegithTexture(strFilePath);
	}
	else if(eType == LOADTYPE::TYPE_BINARY) {
		string strPath;
		strPath.assign(strFilePath.begin(), strFilePath.end());
		m_pGameInstance->Load_Data_Mesh(this, strPath.c_str());
	}
...
HRESULT CVIBuffer_Terrain::Init_HegithTexture(const wstring& strHeightMapFilePath)
{
	_ulong	dwByte = 0;
...
CVIBuffer_Terrain* CVIBuffer_Terrain::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, LOADTYPE eType, const wstring& strFilePath)
{
	CVIBuffer_Terrain* pInstance = new CVIBuffer_Terrain(pDevice, pContext);

	if (FAILED(pInstance->Initialize_ProtoType(eType, strFilePath))) {
