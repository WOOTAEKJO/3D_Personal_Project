...
CMesh_Instancing::CMesh_Instancing(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CVIBuffer_Instancing(pDevice, pContext)
{
}

CMesh_Instancing::CMesh_Instancing(const CMesh_Instancing& rhs)
	: CVIBuffer_Instancing(rhs)
{
}

HRESULT CMesh_Instancing::Initialize_ProtoType(CModel::TYPE eType, MESH MeshData,
	_fmatrix matPivot, CModel::BONES& pBones, MESH_INSTACE_TYPE eBufferType)
{
	...
	{
		for (_uint j = 0; j < MeshData.vecIndices.size(); j++)
		{
...
HRESULT CMesh_Instancing::Initialize(void* pArg)
{
...
_bool CMesh_Instancing::Compute_MousePos(_float3* pOut, _matrix matWorld, _float* fDist)
{
	//_float	fDist = 0.f;
...
HRESULT CMesh_Instancing::Bind_Blend(CShader* pShader, const _char* strName, CModel::BONES& pBones, _int iNonBindInd)
{
...
HRESULT CMesh_Instancing::Anim_Vertex(MESH MeshData, CModel::BONES& pBones)
{
...
HRESULT CMesh_Instancing::NonAnim_Vertex(MESH MeshData, _fmatrix matPivot)
{
...
CMesh_Instancing* CMesh_Instancing::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, CModel::TYPE eType, MESH MeshData,
	_fmatrix matPivot, CModel::BONES& pBones, MESH_INSTACE_TYPE eBufferType)
{
	CMesh_Instancing* pInstance = new CMesh_Instancing(pDevice, pContext);

	if (FAILED(pInstance->Initialize_ProtoType(eType, MeshData, matPivot, pBones))) {
		MSG_BOX("Failed to Created : CMesh_Instancing");
...
CComponent* CMesh_Instancing::Clone(void* pArg)
{
	return nullptr;
}

void CMesh_Instancing::Free()
{
	__super::Free();
}
